'''Задача: Даны несколько списков, состоящих из строк
...
2 В переменную second_result запишите список[] созданный при помощи сборки состоящий из 
...
Пример результата выполнения программы: Пример выполнения кода: print(first_result); print(second_result); print(third_result)
...
first_result= [len(x) for x in first_strings if len(x)>=5]; print(first_result)
...
second_result =[(x,y) for x in first_strings for y in second_strings if len(x)==len(y)]; print(second_result)

# В переменную third_result запишите словарь{} созданный при помощи сборки, где парой ключ-значение будет строка-длина строки. Значения строк будут перебираться из объединённых вместе списков first_strings и second_strings. Условие записи пары в словарь - чётная длина строки.    # объединяем с помощю + (можно с помощью zip и append, но код больше)
third_result={x: len(x) for x in first_strings+second_strings if len(x)%2==0}; print(third_result)
