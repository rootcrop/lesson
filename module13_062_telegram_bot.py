...
kb=ReplyKeyboardMarkup()                # инициализируем клавиатуру через экземпляр ReplyKeyboardMarkup
button1 = KeyboardButton(text='рассчитать')    # делаем кнопку с текстом
button2 = KeyboardButton(text='информация')
kb.add(button1)                          # добавляем кнопку в клавиатуру
kb.add(button2)
...
@dp.message_handler(text=['рассчитать','calories'])      # ждем ввода рассчитать или calories
async def set_age(message):
    await message.answer('введите свой возраст: ')
    #print('введите свой возраст : ')
    await UserState.age.set()               # ждем ввода возраста c помощью метода set в атрибут age
...
async def start (message):
    ...
    await message.answer('привет!\nя бот помогающий твоему здоровью \n\nнажмите /calories или нажмите рассчитать \nдля подсчета ежедневных калорий')
    await message.answer('запускаю меню', reply_markup=kb)  #
