...
@app.get("/admin/{login}/{password}")                      # http://127.0.0.1:8000/user/alex/22
async def user (login:  Annotated[str, Path(min_length=5, max_length=10,
                    description='Enter login, in english or cyrillic letters, digits, "_", "-"',
                    regex="^[a-zA-Z0-9А-Яа-я0-9_-]+$"), ],
                password: Annotated[str, Path(min_length=5, max_length=10,
                    description='Enter password, in english or cyrillic letters, digits, "_", "-"',
                    regex="^[a-zA-Z0-9А-Яа-я0-9_-]+$")]) -> str:
    return f"Ваш логин: {login}"
